...
void XmlGui::parseDefaults(TiXmlElement *element, gcn::Widget *widget)
{
	...
	{
		int b = atoi(element->Attribute("bordersize"));
...
void XmlGui::parseLabel(TiXmlElement *element,gcn::Widget *parent)
{
	...
	{
		std::string align = element->Attribute("align");
...
void XmlGui::parseSlider(TiXmlElement *element,gcn::Widget *parent)
{
	...
	{
		int start  = atoi(element->Attribute("start"));
	...
	{
		int end = atoi(element->Attribute("end"));
		slider->setScaleEnd(end);
	}

	if(element->Attribute("value"))
	{
		int value = atoi(element->Attribute("value"));
	...
	{
		int l = atoi(element->Attribute("stepLength"));
		slider->setStepLength(l);
	}

	if(element->Attribute("orientation"))
	{
		std::string orientation = element->Attribute("orientation");
		...
		{
			slider->setOrientation(gcn::Slider::VERTICAL);
		}
	}
...
void XmlGui::parseScrollArea(TiXmlElement *element,gcn::Widget *parent)
{
	...
	{
		std::string hPolicy = element->Attribute("hPolicy");
		if(hPolicy == "ALWAYS" || hPolicy == "always")
			scroll->setHorizontalScrollPolicy(gcn::ScrollArea::SHOW_ALWAYS);
		else if(hPolicy == "NEVER" || hPolicy == "never")
			scroll->setHorizontalScrollPolicy(gcn::ScrollArea::SHOW_NEVER);
	}

	if(element->Attribute("vPolicy"))
	{
		std::string vPolicy = element->Attribute("vPolicy");
		if(vPolicy == "ALWAYS" || vPolicy == "always")
			scroll->setVerticalScrollPolicy(gcn::ScrollArea::SHOW_ALWAYS);
		else if(vPolicy == "NEVER" || vPolicy == "never")
